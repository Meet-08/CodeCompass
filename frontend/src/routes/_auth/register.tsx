    ...
    <div className="w-full bg-[#0F141E]/90 backdrop-blur-2xl border border-slate-800/80 rounded-3xl p-7 sm:p-9 shadow-[0_0_50px_rgba(0,0,0,0.5)] relative overflow-hidden group">
      ...
      <div className="mb-6 text-left">
        ...
        <p className="text-slate-400 text-sm">
          Join thousands of developers exploring & understanding codebases with
          AI.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-3.5">
          ...
          {password.length > 0 && (
            <div className="mt-2 space-y-1">
              <div className="flex items-center justify-between text-[11px]">
                ...
                <span
                  className={`font-semibold ${strength.score >= 3 ? 'text-emerald-400' : strength.score === 2 ? 'text-amber-400' : 'text-rose-400'}`}
                >
                  {strength.label}
                </span>
              </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            ...
            {confirmPassword.length > 0 && (
              <span
                className={`text-[11px] font-medium flex items-center gap-1 ${passwordsMatch ? 'text-emerald-400' : 'text-rose-400'}`}
              >
                {passwordsMatch ? <Check className="w-3 h-3" /> : null}
        ...
        <div className="flex items-start pt-1">
          <label className="flex items-start gap-2.5 text-xs text-slate-300 cursor-pointer select-none leading-relaxed">
            <input
              type="checkbox"
              required
              checked={agreeTerms}
              onChange={(e) => setAgreeTerms(e.target.checked)}
              className="mt-0.5 w-4 h-4 rounded border-slate-700 bg-slate-900 text-orange-500 focus:ring-orange-500 accent-orange-500 cursor-pointer"
            />
            <span>
              I agree to the{' '}
              <a
        ...
        <button
          ...
          className="w-full mt-2 py-3 px-4 rounded-xl bg-orange-600 hover:bg-orange-500 border border-orange-500/20 text-white font-semibold text-sm shadow-md shadow-orange-950/30 active:scale-[0.99] transition-all flex items-center justify-center gap-2 group cursor-pointer disabled:opacity-70"
        >
